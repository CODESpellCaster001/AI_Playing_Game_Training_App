...
import os
import datetime
from A2C.cnn_architecture import CustomCNN
from Common.game_env import create_env, create_eval_env
from A2C.training_utils import setup_eval_callback, create_model, train_model, save_model
...
def main(env_name, save_dir, log_dir, timesteps, vectorstacks, env_count):
    ...
    log_subdir = f"A2C_{sanitized_env_name}_{current_time}"
    ...
    updated_model_path = os.path.join(
        save_dir,
        f"CustomCNN_A2C_{sanitized_env_name}_{current_time}"
    )
