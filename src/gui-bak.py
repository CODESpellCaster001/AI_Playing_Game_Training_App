...
class BreakoutAIApp:
    ...
    def create_widgets(self):
        ...
        self.algo_label.grid(row=0, column=0, padx=5, pady=5)
        self.algorithm_options = ["DQN", "PPO"]
        self.algo_var = tk.StringVar(self.training_frame)
        self.algo_var.set(self.algorithm_options[0])  # 默认选择 DQN
        self.algo_menu = ttk.Combobox(self.training_frame, textvariable=self.algo_var, values=self.algorithm_options,
                                      state="readonly")
        ...
        # Info label explaining potential caching issue
        self.tensorboard_hint = tk.Label(
            self.tensorboard_frame,
            text="Note: TensorBoard may cache previous runs. "
                 "If the log does not update, try clicking 'Show TensorBoard' again or use a new port.",
            wraplength=400,
            fg="gray"
        )
    ...
    def train_model(self):
        """
        Start the training process for the DQN model in a separate thread and show a message when done.
        """
        ...
        def train_thread():
            ...
            # 训练完成后，在主线程弹窗提示
            self.root.after(0, lambda: messagebox.showinfo("Training Complete", "Model training has finished."))
    ...
    # def _run_tensorboard(self):
    #     """
    #     Run the TensorBoard process in the background and display the output URL in the application.
    #     """
    #     try:
    #         # 后台启动 TensorBoard，不阻塞线程
    #         print(f"[DEBUG] TensorBoard logdir: {self.log_dir}")  # ✅ 打印当前路径到控制台
    #         subprocess.Popen(["tensorboard", "--logdir", self.log_dir], stdout=subprocess.DEVNULL,
    #                          stderr=subprocess.DEVNULL)
    #
    #         # 在 GUI 中显示提示文字
    #         self.tensorboard_output.config(text="TensorBoard is running at: http://localhost:6006")
    #
    #         # 可选：自动打开浏览器
